...
func (fh *FileHandle) ReadFile(offset int64, buf []byte) (bytesRead int, err error) {
	...
	for bytesRead < noWant && err == nil {
		noRead, _ = fh.readFile(offset+int64(bytesRead), buf[bytesRead:])
		if noRead > 0 {
			bytesRead += noRead
		}
	}
...
func (fh *FileHandle) readFile(offset int64, buf []byte) (bytesRead int, err int) {
	defer func() {
		if bytesRead > 0 {
...
func (fh *FileHandle) readFromStream(offset int64, buf []byte) (bytesRead int, err int) {
	defer func() {
	...
	var err2 error
	bytesRead, err2 = fh.reader.Read(buf)
	if err2 != nil {
		if err2 != io.EOF {
