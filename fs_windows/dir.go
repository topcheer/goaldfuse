...
func (inode *Inode) OpenDir() (dh *DirHandle) {
	...
	inode.fs.mu.Lock()
	inode.fs.dirHandles[HandleID(dh.inode.fileHandles)] = dh
	inode.fs.mu.Unlock()
	return
}
...
func (dh *DirHandle) ReadDir() (en []*DirHandleEntry, err error) {
	...
	//add dot
	c := &DirHandleEntry{Name: ".", Type: DT_Directory, Inode: parent.Id, Offset: 0}
	en = append(en, c)
	//add dotdot
	if parent.Parent != nil {
		p := &DirHandleEntry{Name: "..", Type: DT_Directory, Inode: parent.Parent.Id, Offset: 1}
		en = append(en, p)
	} else {
		p := &DirHandleEntry{Name: "..", Type: DT_Directory, Inode: 0, Offset: 1}
		en = append(en, p)
	}
	...
	for i, item := range dirList.Items {
		...
		if inode := parent.findChildUnlocked(item.Name, item.Type); inode != nil {
			...
			if item.Type == "folder" {
				...
				entry.Attributes = InodeAttributes2{Size: uint64(item.Size),
		...
		e := &DirHandleEntry{Name: ety.Name, Inode: ety.Id, Type: typ, Offset: DirOffset(uint64(i + 2))}
		en = append(en, e)
	}
	return en, nil
...
func (parent *Inode) Unlink(name string) (err int) {
...
func (parent *Inode) Create(
	name string, metadata OpContext) (inode *Inode, fh *FileHandle) {
...
func (parent *Inode) MkDir(
	name string) (inode *Inode, err int) {
...
func (parent *Inode) RmDir(name string) (err int) {
	parent.logFuse("Rmdir", name)
	inode := parent.findChildUnlocked(name, "folder")
	if inode == nil {
		return fuse.ENOENT
	}
	isDir, errs := inode.isEmptyDir()
	if errs != nil {
		return
	}
